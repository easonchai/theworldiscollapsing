...
| ⚠ OpenRouter markup over MiniMax | Not published on any fetched page | none |
...
| `openai/gpt-5-nano` | 400k | $0.05 | $0.40 | yes | yes, for tickers/cards |
...
| Timelock-sealed prediction market in production | None found | none |
...
| Hedera $15k, Arc $10k | other L1s | n/a | Rejected |
| 1inch $7k, Uniswap $5k | AMM tooling | n/a | Rejected (contradicts parimutuel) |
| ENS $5k | Ethereum Sepolia only | n/a | Rejected |
| Ledger $5k, Bazantic $3k | n/a | n/a | Rejected |

- No Base sponsor. No prediction-market track. No video sponsor. "prediction" appears 0 times in prize text.
- Event: **Sept 4–16, 2026**, async. ⚠ Submission cutoff not retrievable (event page HTTP 500). Check the dashboard.
...
| Download | `GET /api/v1/videos/{id}/content?index=0` with `Authorization: Bearer`, the same URL as `unsigned_urls[0]` | same |
...
This confirms the plan's cost model exactly: **480p $0.05/s, 768p $0.08/s**, clips 5–15 s, first/last frame image-to-video supported. `seed: false`, so no determinism on this model.
...
| ⚠ Derived children inside a collection query | `events(...) { markets { ... } }` (a `@derivedFrom` list selected inside a parent collection) is not spelled out on that page. The markets list depends on it; it was exercised only against a stubbed endpoint, never against a real graph-node. | none |
...
| ⚠ Local reachability | Never exercised: no gateway API key, nothing published. The server addresses deployments on The Graph Network, so a `localhost:8000` graph-node is not reachable from it. | none |
...
| Runtime | `TeeRuntime<C> extends BaseRuntime<C>, SecretsProvider`, with `config`, `now()`, `log()`, `emitMetric()`, `getSecret()`, `getSecrets()`, `reportFromDon()`, `usingTheDons()` | same |
...
| Pairing precompile 0x11 | rejects empty input (`PrecompileError`), as expected: EIP-2537 requires a non-empty multiple of 384 bytes | same |
...
### Option B, **chosen**: switch the beacon to drand `evmnet` (BN254)

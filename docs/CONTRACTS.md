...
- `POST /internal/reveal-key` on the engine media server (`MEDIA_PORT`), `Authorization: Bearer $REVEAL_SECRET` (constant-time compare), body `{ eventId, outcome, key }` with `key` as 0x-hex → decrypts `branch-<outcome>.mp4.enc` in place, rewrites `Event.branchUrls[outcome]` to the plaintext URL, returns `{ url }`. 401 on a bad token, 400 on a bad key (GCM tag) or malformed body. The endpoint only exists when `BRANCH_SEAL=1`.

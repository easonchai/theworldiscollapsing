...
- **Subgraph** is a derived index of chain for the markets list, positions and stats. No mocks: when `NEXT_PUBLIC_SUBGRAPH_URL` is unset the pages that need it render an explicit "subgraph not configured" state. Only `/markets` and `/positions` read it (`apps/web/src/components/markets-list.tsx`, `positions-list.tsx`); the wall tiles and the event page read pools from `Arena` with a viem public client (`readMarket` in `apps/web/src/components/markets.tsx`). The local run in `docs/LOCAL.md` does not start graph-node, so on that stack live pools are proved on `/` and `/e/<id>`, never on `/markets`: the markets and positions pages correctly stay "not configured" until you run the graph-node block in `docs/LOCAL.md` "Local subgraph" (the same steps are in the graph-node row of "Local stack" below) and set the var. An empty `/markets` on the default stack is the documented state, not a defect. A checklist that asserts pools there is checking the wrong page: check the wall tile and the event page, or do the graph-node bring-up first.
...
- `LOCKED`: lock passed; waiting for the drand round. Round time = `1727521075 + (round − 1) × 3` (unix seconds), off the drand **evmnet** genesis; see "Randomness" below.
...
type EventPublic = {
  ...
  score: { sides: [string, string]; score: string; final: boolean } | null; // corner scorebug, sports only
...
- **House style.** Two places, both exported and unit-tested: `CHANNEL_STYLE` (`apps/engine/src/author.ts`) is the per-channel Subject / Camera / On screen / Pacing block in the authoring system prompt, and `clipPrompt(channelId, shot)` (`apps/engine/src/render.ts`) wraps every prompt that reaches the video vendor as `CHANNEL_PREFIX[channel]` + the authored shot + `STYLE_SUFFIX` ("Real-time speed. No slow motion. Not cinematic. No film look. Natural light as it is." plus the no-readable-text rule below), trimmed to 800 chars. It is on the single `clip()` path, so first half, branches and `keyArtPrompt` all go through it. MiniMax has no negative-prompt field: constraints are plain positive sentences, never a negative list.
- **No readable text in frame.** The first live render of the v2 suffix put a gold awards screen reading `CHENNIU DORSTAWS RAPOOI` at the centre of the culture clip, a broadcaster watermark on region, and garbled hoardings on sports. "No captions, no logos" reads to the model as a rule about overlays, so signage, screens, hoardings, watermarks and printed clothing all survived it, and fast-h3 renders every one of them as nonsense letterforms. The v3 suffix names what the camera sees instead: signs and screens may be in frame, but their lettering is too small, too distant or too oblique to read, and no readable words appear anywhere in frame. Signage is not banned outright, because politics needs a chart on the studio screen and the shape is what carries it. The wording avoids "out of focus" and "blurred", which ask for a soft image rather than an unreadable one.

  The v3 suffix rendered on 2026-09-12 and lost. The authored shot said "stage lights and award logo visible", so the prompt asked for a logo and then forbade one, and the video model sided with the concrete thing the shot named. The suffix stays, but the rule that does the work now sits in the authoring prompt: shot text may not name a logo, sign, banner, step-and-repeat backdrop, scoreboard, ticker, lower third, name card, hoarding or printed slogan. Politics is the single exception and it is a shape rather than words, a rising bar or a red zone on a map, with the numbers on it free to be illegible. Culture also carries the instruction in its own house style, because it produced the worst failure: frame the people, never the backdrop behind them.
- **Titles and outcome labels carry no numbering.** These are the two strings a viewer reads next to their money, and the author prompt's own scaffolding leaked into them: two of four REAL events came back as `Event 43 — National Film Gala` and `Outcome 1 — Harbour City win`. The prompt asks for the name alone, and `author.ts` strips a leading `Outcome N —` / `Event N:` prefix afterwards, the same way it clamps shot lengths rather than trusting the model to obey. A separator is required, so "Option B holds" and "Eventual Recount" survive untouched. Asking for the name alone then produced the opposite failure, `Nominee A / Nominee B / Nominee C`, which misreads just as badly by being vague. The prompt now asks for the world's real names the way the canon writes them, and a probe across all four channels came back with "Aria Solace wins Album of the Year" and "Harbour City win".
...
- **Scorebug** (sports only): `Authored.score` is `{ sides: string[2]; atBreak: string; atEnd: string[] }`, one final per outcome in the same order as `outcomes`, enforced by a refine. It exists because the video model cannot draw letterforms, so the one fact a bettor has to read off the picture is page text like the cards and the ticker, never pixels. `sides` is a plain array rather than a tuple: a `z.tuple` compiles to `prefixItems`, which OpenAI structured outputs rejects with "array schema items is not an object", failing the whole authoring call. The channel is enforced in `makeAuthor`, not merely asked for in the prompt, because the schema offers the field to every channel and a live probe came back with a region scorebug reading `City v Harb` and finals in Chinese characters. `atEnd` is the same class of secret as `branchUrls`: `toPublic` sends `atBreak` before the reveal and only `atEnd[outcome]` after it, as `EventPublic.score.score`, so the response never carries the losing finals. `apps/web/src/components/event-stage.tsx` draws it in the frame's top-right corner, opposite the UMD.
...
- `address(0)` is **trusted mode**, the pre-day-7 behaviour: the signature is stored as submitted and
...
- with a `DrandVerifier`, `resolve()` reverts `BadSignature` unless the BLS signature verifies against the
...
**Every `POST /api/verify` call must carry a wallet signature.** `message` is exactly `theworldiscollapsing verify <address> <unixSeconds>`, the timestamp must be within 5 minutes of the server clock, and viem `verifyMessage` must recover `address`. Without it anyone could spend the owner's gas verifying addresses they do not control. The route also rate-limits to one verification per address per minute **and** caps the whole instance at 30 `setVerified` transactions an hour (`verifyGasCap` in `apps/web/src/lib/limits.ts`, 429 beyond it), because the per-address limit alone is bypassed by generating fresh addresses, and every one of those costs the owner gas. World Selfie Check uses `@worldcoin/idkit` 4.2 on the client → proof → `POST /api/verify` → World verify endpoint per the live docs → `setVerified`. The widget is given `signal = address`, so every credential response carries `signal_hash = hashSignal(address)`; the route re-derives that hash **from the signed address, never from a client-supplied signal** and refuses a proof that does not carry it (`proofBoundTo` in `apps/web/src/lib/world.ts`), which is what stops one Selfie Check from verifying any number of addresses. See docs/RESEARCH.md for the field-by-field source. 18+ is a self-attest checkbox at signup in both modes. Checkbox mode: `POST /api/verify { address, attest: true }` → `setVerified`. Faucet and bet are gated on chain; the UI explains why and how to fix it when a user is unverified. **Smart wallets and gas.** With smart wallets on in the Privy dashboard (docs/SETUP.md §7) the user's address is a Privy smart wallet and the dashboard's paymaster pays its gas; the route then verifies the signature on chain via `publicClient.verifyMessage` (ERC-1271, ERC-6492 while undeployed) after the offline ECDSA check fails. **Otherwise the route pays for gas.** A Privy embedded wallet is created with no ETH, so a verified address whose balance is under `GAS_MIN` (0.0005 ETH) gets `GAS_DRIP` (0.002 ETH) from the owner, counted under the same hourly cap; the response carries the drip as `gas`. `ensureGas` (`apps/web/src/lib/tx.ts`) runs before every client write (faucet, approve/bet, claim), skips sponsored wallets, and otherwise re-signs the challenge to ask for a top-up when the balance is low, so an already-verified address never has to find the verify page again.
...
`pnpm --filter engine bettor` (`apps/engine/scripts/bettor.ts`, tsx + viem) keeps every open market busy: it discovers `BETTING` events across all channels through Prisma and, until three seconds before each `lockTime`, places bets with a random bettor, market, side and log-uniform amount, so no market ends up void under `Arena.claim`. **Coverage is the void ratio, not "both sides non-zero"**: the planner (`apps/engine/src/bettor-plan.ts`, pure and seeded) sizes every bet so each side keeps at least its `1/nOutcomes` share, fills every short (market, side) slot before spending a tick on volume, and skips a slot it cannot fill rather than under-funding it. It funds, verifies, faucets and approves each bettor first, then claims once the event resolves and prints payouts and per-bettor P&L. Perpetual until SIGINT/SIGTERM; `--events N` stops after N events have been bet and claimed. It loads `apps/engine/.env` itself. Start it **before** the engine: it only bets on windows it is running for, and it only claims events it opened in its own process.

Env (flags `--rpc/--arena/--usdc/--gate/--db/--owner/--keys/--events` override): required `BETTOR_KEYS` (comma-separated private keys), `GATE_OWNER_PRIVATE_KEY` (the Gate owner, anvil account 0 locally and the deployer on testnet, which also verifies the bettors and pays their gas out of its own balance), `RPC_URL`, `CHAIN_ID`, `ARENA_ADDRESS`, `USDC_ADDRESS`, `GATE_ADDRESS`, `DATABASE_URL`. Optional: `BETTORS` (how many keys to use, default all), `BET_MIN_USDC` (25), `BET_MAX_USDC` (30), `BET_INTERVAL_MS` (3000, the *slowest* gap; the plan jitters 0.6x–1.4x and speeds up on its own when the slots left do not fit in the window, floor 200 ms), `FUND_MIN_ETH` (0.005), `FUND_ETH` (0.01), `BETTOR_SEED` (default time-based; the same seed replays the same bets), `POLL_MS` (receipt polling: 200 ms on anvil, 2000 ms elsewhere; the engine applies the same rule without a knob, `apps/engine/src/chain.ts`).

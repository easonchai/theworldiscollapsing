...
Env (flags `--rpc/--arena/--usdc/--gate/--db/--owner/--keys/--events` override): required `BETTOR_KEYS` (comma-separated private keys), `GATE_OWNER_PRIVATE_KEY` (the Gate owner — anvil account 0 locally, the deployer on testnet — which also verifies the bettors and pays their gas out of its own balance), `RPC_URL`, `CHAIN_ID`, `ARENA_ADDRESS`, `USDC_ADDRESS`, `GATE_ADDRESS`, `DATABASE_URL`. Optional: `BETTORS` (how many keys to use, default all), `BET_MIN_USDC` (25), `BET_MAX_USDC` (30), `BET_INTERVAL_MS` (3000 — the *slowest* gap; the plan jitters 0.6x–1.4x and speeds up on its own when the slots left do not fit in the window, floor 200 ms), `FUND_MIN_ETH` (0.005), `FUND_ETH` (0.01), `BETTOR_SEED` (default time-based; the same seed replays the same bets), `POLL_MS` (receipt polling: 200 ms on anvil, 2000 ms elsewhere — the engine applies the same rule without a knob, `apps/engine/src/chain.ts`).
